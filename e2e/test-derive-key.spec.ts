...
test('Check C_DeriveKey on WASM', async ({ page }) => {
  ...
  await page.evaluate(() => localStorage.setItem('pqc-version-storage', JSON.stringify({ state: { hasSeenToast: true }, version: 0 })));
  await page.reload();
  ...
  await expect(page.locator('body')).toContainText(/PKCS#11|SoftHSM|Live WASM|Sign /i, { timeout: 15000 });
