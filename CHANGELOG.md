...
- **Library "Relevant Features" links broken for many documents** [view:/library]: The `DimensionLinks` component in `DocumentAnalysis` used case-sensitive lookup against `FEATURE_ROUTES`, but enrichment data contains mixed casing (`compliance` vs `Compliance`, `Hybrid-crypto` vs `hybrid-crypto`). Replaced with case-insensitive `Map`-based lookup so all feature tags render as clickable navigation links regardless of casing in the enrichment source.
