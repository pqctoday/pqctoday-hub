 ...
 * In dev mode, also checks the dynamic test vendor generated by the SDK test
 * site (cert stored in localStorage) and the static test fixture certs.
 *
...
export async function findVendor(kid: string): Promise<VendorRegistryEntry | undefined> {
  // Dev-only: check dynamic + static test entries first (localStorage-backed)
  if (import.meta.env.DEV) {
    try {
      const devModule = await import('./vendorRegistry.dev')
      const devEntry = devModule.findTestVendor(kid)
      if (devEntry) return devEntry
    } catch {
      // vendorRegistry.dev.ts doesn't exist — fine
    }
  }
