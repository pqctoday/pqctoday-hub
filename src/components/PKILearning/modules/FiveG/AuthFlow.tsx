...
export const AuthFlow: React.FC<AuthFlowProps> = ({ onBack }) => {
  const steps: Step[] = FIVE_G_CONSTANTS.AUTH_STEPS.map((step, index) => ({
    ...
    diagram: <AuthDiagram step={index} />,
  }))
...
Step 1: Input Parameters
  > K:    [from USIM]
  > OPc:  [operator constant]
  > RAND: ${vec.rand}
  > SQN:  [sequence number]
  > AMF:  [auth management field]

Step 2: Executing MILENAGE Functions (software AES-128)
  > f1 (MAC-A):  ${vec.mac}
  > f2 (XRES):   ${vec.res}
  > f3 (CK):     ${vec.ck}
  > f4 (IK):     ${vec.ik}
...
Note: MILENAGE uses AES-128 in a proprietary construction (not CBC).
No dedicated PKCS#11 v3.2 mechanism exists for 5G-AKA.
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-muted/40 text-xs text-muted-foreground">
        ...
        <span>
          ...
          not for production systems. PKCS#11 v3.2 has no dedicated 5G-AKA mechanism; this
          simulation uses software MILENAGE (AES-128).
        </span>
      </div>
      <StepWizard
        steps={steps}
