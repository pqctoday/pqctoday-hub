...
export const Introduction: React.FC<IntroductionProps> = ({ onNavigateToWorkshop }) => {
  return (
    <div className="space-y-6">
      ...
      <div data-section-id="quantum-threats-platform" className="scroll-mt-20">
        <CollapsibleSection
        ...
        >
          <p className="text-muted-foreground">
            ...
            signing keys for container images, RSA-2048 certificates for TLS sessions to Vault and
            the Kubernetes API server, and HMAC-SHA-256 for webhook secrets. A cryptographically
            relevant quantum computer (CRQC) running{' '}
          ...
          <div className="bg-status-error/10 rounded-lg p-4 border border-status-error/20">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              kube-apiserver traffic, registry pulls — and storing them for decryption once a CRQC
              is available. Even if your certificates expire in 90 days, the data they protected may
              remain sensitive for 10–20 years. This makes TLS key exchange (ECDHE) the most
              critical asset to migrate, ahead of signing keys.
            </p>
          </div>
        </CollapsibleSection>
      </div>
      ...
      <div data-section-id="crypto-asset-discovery" className="scroll-mt-20">
        <CollapsibleSection
      ...
      <div data-section-id="container-signing" className="scroll-mt-20">
        <CollapsibleSection
          title="3. Container &amp; Artifact Signing: ECDSA → ML-DSA"
          icon={<Container size={20} className="text-primary" />}
        >
          <p className="text-muted-foreground">
            Container image signatures anchor the entire supply chain trust model. If the signing
            key is broken, every image in your registry can be forged. Current tools —
      ...
      <div data-section-id="iac-crypto-config" className="scroll-mt-20">
        <CollapsibleSection
        ...
        >
          <p className="text-muted-foreground">
            ...
            years without active change. Terraform, Helm, and cert-manager all default to RSA-2048
            or ECDSA P-256 — every resource provisioned without explicit PQC configuration is a
          ...
          <div className="space-y-3">
            {[
              ...
              },
              {
                ...
                problem:
                  'CI/CD deploy certs are ECDSA P-256. All kubectl sessions are HNDL targets.',
      ...
      <div data-section-id="policy-enforcement" className="scroll-mt-20">
        <CollapsibleSection
        ...
        >
          ...
          <div className="space-y-2 text-sm text-muted-foreground">
            ...
            <div className="flex items-start gap-2">
              <ArrowRight size={14} className="text-primary shrink-0 mt-0.5" />
              <span>
                <strong className="text-foreground">Kyverno ClusterPolicy</strong> — verifyImages
                rule requires ML-DSA-65 cosign or Notation signature on all Pods in production
                namespace. Blocks unsigned or ECDSA-only images.
              </span>
            </div>
            <div className="flex items-start gap-2">
              <ArrowRight size={14} className="text-primary shrink-0 mt-0.5" />
              <span>
                <strong className="text-foreground">
                  SLSA Level 3 (projected PQC requirement)
                </strong>{' '}
      ...
      <div data-section-id="monitoring-posture" className="scroll-mt-20">
        <CollapsibleSection
          title="6. Monitoring Cryptographic Posture"
          icon={<Activity size={20} className="text-primary" />}
        >
          <p className="text-muted-foreground">
            Continuous monitoring detects algorithm drift — when classical certs re-enter the
            cluster through CA fallback, manual issuance, or operator error. Four Prometheus-based
            exporters cover the full platform crypto surface.
          </p>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            cross-platform detection — catching algorithm-downgrade events, CI/CD OIDC token
            algorithm drift, and TLS cipher negotiation without ML-KEM.
          </p>
        </CollapsibleSection>
      </div>

      {/* Section 7: Migration Runway */}
      <div data-section-id="migration-runway" className="scroll-mt-20">
        <CollapsibleSection
        ...
        >
          <p className="text-muted-foreground">
            ...
            clients that haven&apos;t yet updated. The six phases progress from inventory to
            complete cut-over, with explicit rollback procedures at each step.
          </p>
          ...
          <div className="space-y-2">
            {[
              ...
              {
                phase: 1,
                title: 'Root CA Migration (ML-DSA-65)',
                weeks: '4w',
                priority: 'critical',
              },
              {
          ...
          <p className="text-sm text-muted-foreground">
            ...
            Key Exchange) address HNDL directly and should begin immediately, regardless of your
            CRQC timeline estimate. Even if a CRQC is 15 years away, encrypted traffic recorded
            today will be decryptable in 2039.
          </p>
