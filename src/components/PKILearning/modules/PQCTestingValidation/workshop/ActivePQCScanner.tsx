...
import { ScanSearch, ShieldCheck, ShieldAlert, ShieldOff, AlertTriangle, CheckCircle } from 'lucide-react'
import {
  SCAN_TARGETS,
  SCAN_RESULTS,
  type ScanTarget,
...
const PQC_BADGE: Record<ScanResult['pqcSupport'], { label: string; cls: string; icon: React.ReactNode }> = {
  full: { label: 'Quantum-Safe', cls: 'bg-status-success/10 text-status-success border-status-success/30', icon: <ShieldCheck size={12} /> },
  hybrid: { label: 'Hybrid PQC', cls: 'bg-status-info/10 text-status-info border-status-info/30', icon: <ShieldAlert size={12} /> },
  none: { label: 'Vulnerable', cls: 'bg-destructive/10 text-destructive border-destructive/30', icon: <ShieldOff size={12} /> },
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20">
        ...
        <p className="text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">Simulating:</span> pqcscan (Anvil Secure) active
          endpoint probe. Sends TLS/SSH/IKEv2 handshake to each target and inspects negotiated
          algorithms.{' '}
          <span className="text-status-warning">Active mode — connection attempts made to each target.</span>
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex gap-2">
          ...
          {!scanned ? (
            ...
            >
              {scanning ? 'Scanning…' : `Scan ${selected.size} target${selected.size !== 1 ? 's' : ''}`}
            </button>
          ) : (
            <button
              onClick={resetScan}
          ...
          {filteredTargets.map((t) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="font-mono text-xs font-semibold text-foreground">{t.hostname}</span>
                <span className={`text-xs px-1.5 py-0.5 rounded border font-mono ${PROTO_BADGE[t.protocol]}`}>
                  :{t.port}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-xs px-1.5 py-0.5 rounded border uppercase font-semibold ${PROTO_BADGE[t.protocol]}`}>
                  {t.protocol}
                </span>
      ...
      {scanning && (
        <div className="flex flex-col items-center gap-3 py-8">
          ...
          <p className="text-sm text-muted-foreground">Probing {selected.size} endpoint{selected.size !== 1 ? 's' : ''}…</p>
        </div>
      )}
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-xs px-1.5 py-0.5 rounded border uppercase font-semibold ${PROTO_BADGE[target.protocol]}`}>
                          {target.protocol}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {r.hybridAlgorithm && (
                          <span className="text-xs font-mono text-muted-foreground">{r.hybridAlgorithm}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      <span className={`text-2xl font-bold tabular-nums ${RISK_COLOR(r.riskScore)}`}>
                        {r.riskScore}
                      </span>
