...
- **Interactive Playground**: Hands-on cryptographic testing environment
  ...
  - **TPM 2.0 Playground** (`/playground` → TPM tab): in-browser TCG TPM 2.0 V1.85 emulation via a custom WASM build of libtpms (pqctoday-tpm). Supports the full V1.85 PQC key-hierarchy lifecycle: `TPM2_Startup`, `TPM2_SelfTest`, `TPM2_GetCapability`, `TPM2_GetRandom`, `TPM2_CreatePrimary` (ML-KEM-768 EK + ML-DSA-65 AK), `TPM2_Encapsulate`, `TPM2_Decapsulate`, `TPM2_SignDigest`. Semantic command builder with live hex preview; object store shows loaded transient handles; wire-accurate hex stream synchronized to the parameter table.
    - **TCG V1.85 Compliance Runner**: 16/16 automated checks (V185-001 through V185-016) covering self-test, response header, algorithm registration (ML-KEM 0x00A0, ML-DSA 0x00A1), entropy, key creation (pk sizes: 1184B / 1952B), encapsulation output (ss=32B, ct=1088B per FIPS 203), decapsulation, and signature size (3309B per FIPS 204 ML-DSA-65). Results update live with pass/fail badges and spec section references.
    - **Scenario Flow tab**: 9-phase narrative trace alongside the compliance checklist — shows the exact TPM2B byte stream sent and received for each command, with dynamic byte counts, handle values, and a PQC key-hierarchy summary table.
